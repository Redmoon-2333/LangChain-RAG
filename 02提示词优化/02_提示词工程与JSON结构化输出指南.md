...
1. **OpenAI库基础** → 了解API调用、消息结构、流式输出
2. **Python基础** → 字典、列表、字符串操作
3. **JSON格式** → 了解JSON的基本结构和语法
...
**核心概念**：**System消息**是提示词的"角色设定"，决定了模型的基本行为模式。
...
**核心概念**：**Few-Shot Learning（少样本学习）**是提示词工程的核心技术，通过在提示词中提供少量示例，让模型快速学习任务模式。
...
**本节要点**：
1. Few-Shot通过示例让模型快速学习任务模式
2. 示例要覆盖所有类别、正负例平衡、包含边界案例
3. 示例数量根据任务复杂度调整，避免过多或过少
...
**本节要点**：
1. 分类任务：明确类别、定义默认处理、示例覆盖全面
2. 信息抽取：定义Schema、指定JSON格式、处理缺失字段
3. 文本匹配：使用分隔符、正负例平衡、输出简洁
...
**本节要点**：
1. JSON是结构化输出的首选格式
2. 明确指定JSON格式、定义结构、使用真实示例
3. 使用`ensure_ascii=False`保持中文可读性
...
**本节要点**：
1. 分类任务：角色清晰、类别完整、默认处理
2. 信息抽取：Schema明确、JSON格式、缺失处理
3. 文本匹配：分隔符清晰、正负例平衡、输出简洁
...
def build_classification_prompt(domain, categories, default_category, examples):
    ...
    Why: 模板化提高代码复用性，减少重复编写
    Warning: 确保示例质量，低质量示例会导致模型表现下降
    
    Args:
        domain: 领域名称
        categories: 类别列表
        default_category: 默认类别
        examples: 示例数据列表 [(text, category), ...]
    """
    messages = [
        {"role": "system", 
...
def self_consistency(client, question, num_samples=5):
    ...
    Why: 多次采样减少随机性带来的错误，提高答案可靠性
    Warning: 会增加API调用次数和成本，适用于关键任务
    """
    answers = []
...
| 错误现象 | 可能原因 | 解决方案 |
|---------|---------|---------|
| 输出格式不一致 | 格式约束不明确 | 增加JSON示例 |
| 分类错误 | 类别边界模糊 | 明确类别定义 |
| 信息遗漏 | Schema不完整 | 列出所有字段 |
| 示例无效 | 示例质量低 | 检查示例准确性 |
