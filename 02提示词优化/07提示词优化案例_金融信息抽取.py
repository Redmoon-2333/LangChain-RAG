"""
文件功能：Few-Shot + JSON结构化信息抽取（与05对比：输出JSON对象而非类别标签）
"""
...
questions = [
    ...
    "2025-06-06，股市利好。股票黑马程序员A股今日开盘价200人民币，一度飙升至211人民币，随后回落至201人民币，最终以206人民币收盘。"  # 缺少"成交量"字段
]

# 【差异】与05相比，system要求JSON格式输出
messages = [
    {"role": "system", "content": f"你帮我完成信息抽取，我给你句子，你抽取{schema}信息，按JSON字符串输出，如果某些信息不存在，用'原文未提及'表示，请参考如下示例："}
]

# 使用json.dumps()将答案序列化为JSON字符串
for example in examples_data:
    messages.append({"role": "user", "content": example["content"]})
    messages.append({"role": "assistant", "content": json.dumps(example["answers"], ensure_ascii=False)})  # ensure_ascii=False: 保留中文
